...
app.setName('Forget');  // egen datamapp, skild från Vaka/Skugga
const path = require('path');
const fs = require('fs');
/* ── FORGET: allt hamnar i RAM och raderas när appen stängs ──────────────── */
const forget = require('./forget');
const FORGET = forget.begin(app);          // flyttar userData/downloads till engångsmappen
forget.lockdownNetwork();                  // inga molnanrop förbi Tor
app.commandLine.appendSwitch('disk-cache-size', '1');   // Chromium ska inte spara cache
...
/* ── Tor: HELA Forget går genom Tor-nätverket ──────────────────────────────
...
// Onion-sök (Ahmia) — samma motor som skalet använder för all sökning.
const AHMIA = 'http://juhanurmihxlp77nkq76byazcldy2hlmovfu2epvl5ankdibsot4csyd.onion/#fq=';
const forgetUpdater = require('./updater');
 ...
 * och minns bara URL:en. Ljudande flikar lämnas ifred. */
function discardTab(ctx, tabId) {
  const v = ctx.views.get(tabId);
  if (!v || tabId === ctx.visibleTab) return;
...
function buildContextMenu(ctx, wc, params) {
  ...
  if (params.isEditable) {
    ...
    t.push({ label: T('Sök anonymt efter') + ` "${short}"`, click: () => sendTo(ctx, 'open-new-tab', AHMIA + encodeURIComponent(sel)) });
    t.push({ type: 'separator' });
  }
  ...
  // Forget: ingen Google Översätt — den skulle skicka adressen du läser till Google.
  t.push({ type: 'separator' });
...
function buildShellMenu(ctx, wc, params) {
  ...
  if (params.isEditable) {
    ...
    t.push({ label: T('Nytt fönster'), click: () => createWindow() });
...
function ensureView(ctx, tabId) {
  ...
  // Forget kör ALLA flikar i samma session — den ligger i RAM och rensas ändå
  // vid stängning, så det finns inget separat inkognitoläge.
...
async function scanDownload(rec) {
  ...
  const cloud = null;   // Forget: ingen moln-hash-koll — filnamn/hash/URL får aldrig lämna maskinen
  const local = await clamScanFile(rec.path);           // lokal ClamAV (om finns)
...
function trackDownloads(sess) {
  sess.on('will-download', (_e, item) => {
    ...
    broadcast('download-update', rec);
    if (downloads.length === 1) broadcast('toast', 'Filen sparas i sessionsmappen — flytta den om du vill behålla den, annars raderas den när du stänger fönstret.');
...
ipcMain.on('view:destroy', (e, id) => {
  ...
  ctx.discarded.delete(id); ctx.lastActive.delete(id);
...
ipcMain.on('open-app-menu', (e) => {
  ...
  const t = [
    ...
    { label: T('Nytt fönster'), accelerator: 'CmdOrCtrl+N', click: () => createWindow() },
    { type: 'separator' },
...
function handleShortcut(wc, input) {
  ...
  if (mod && !shift && k === 'n') { createWindow(); return true; }
...
function createWindow() {
  ...
  // RAM och raderas när fönstret stängs, så ett läge "utan spår" vore samma sak.
  ...
  win.webContents.once('did-finish-load', () => {
    if (pendingUrl) { sendTo(ctx, 'open-new-tab', pendingUrl); pendingUrl = null; }
  });
...
app.whenReady().then(async () => {
  ...
  // Forget: fetch är redan spärrad av forget.lockdownNetwork() — inga egna anrop alls.
  ...
  // Forget uppdateras genom Tor (updater.js) — electron-updater har en egen
  // HTTP-stack som struntar i Chromiums proxy och hade läckt din riktiga IP.
  forgetUpdater.start(broadcast);
});
...
function checkMacUpdate() {
  ...
  if (!f) return;
  const ping = () => f('https://api.github.com/repos/northcrafto/forget-dl/releases/latest', { headers: { 'User-Agent': 'Forget' } })
    .then((r) => r.json())
    .then((j) => {
      const latest = ((j && j.tag_name) || '').replace(/^v/, '');
      if (latest && semverGt(latest, app.getVersion())) {
        broadcast('toast', 'Ny version av Forget (' + latest + ') finns.');
      }
    }).catch(() => {});
...
ipcMain.on('update:install', () => { try { forgetUpdater.install(); } catch {} });
ipcMain.handle('update:check', () => forgetUpdater.check(true));
ipcMain.handle('update:download', () => forgetUpdater.download());
ipcMain.handle('update:state', () => forgetUpdater.getState());
/* ── Forget: avslutningen. Tor och alla Chromium-barn måste vara döda innan
 * sista raderingen, annars skriver de tillbaka filer i mappen vi just tömde. */
let forgetShutdownDone = false;
function forgetShutdown() {
  try { tor.stop(); } catch {}
  try { forget.wipe(session); } catch {}           // rensa sessionsdata + radera mappen
}
app.on('before-quit', forgetShutdown);
app.on('will-quit', forgetShutdown);
app.on('quit', () => { forgetShutdownDone = true; try { forget.wipe(null); } catch {} });
process.on('exit', () => { try { tor.stop(); } catch {} try { forget.wipe(null); } catch {} });   // sista utvägen
for (const sig of ['SIGINT', 'SIGTERM', 'SIGHUP']) {
  process.on(sig, () => {
    forgetShutdown();
    try { app.quit(); } catch {}
    // Om Electron inte hinner stänga ner snyggt: radera igen och tvinga slut.
    setTimeout(() => { try { forget.wipe(null); } catch {} process.exit(0); }, 1200).unref();
  });
}
