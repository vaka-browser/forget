...
/* Forget har inget inkognitoläge — hela webbläsaren är ett. Sessionen ligger i
   RAM och raderas när fönstret stängs, så ett separat "läge utan spår" vore bara
   samma sak en gång till (och skulle antyda att det VANLIGA läget lämnar spår). */

/* ── URL-hjälp ── */
// Forget: sökmotorernas .onion-tjänster används där de finns, så att sökningen
...
const ENGINES = {
  // Standard i Forget: Ahmia — söker BLAND .onion-sajter, och söksidan är själv
  // en onion-tjänst, så frågan lämnar aldrig Tor-nätverket.
  // Frågan ligger i hashen (#fq=) — den skickas ALDRIG till servern. En liten
  // brygga i content-preload.js fyller i Ahmias formulär och skickar det, för
  // deras söksida kräver ett dolt fält som bara deras egen JS sätter.
  onion: { label: 'Ahmia (onion-sök)', url: 'http://juhanurmihxlp77nkq76byazcldy2hlmovfu2epvl5ankdibsot4csyd.onion/#fq=' },
  duckduckgo: { label: 'DuckDuckGo (.onion)', url: 'https://duckduckgogg42xjoc72x3sjasowoarfbgcmvfimaftt6twagswzczad.onion/?q=' },
...
// Forget: DuckDuckGo (.onion) som standard — fungerar rent över Tor (Google spärrar
// Tor-utgångar med captcha) och spårar dig inte.
let searchEngine = 'onion';
try {
  if (!localStorage.getItem('forget-engine-init')) { localStorage.setItem('skoll-engine', 'onion'); localStorage.setItem('forget-engine-init', '1'); }
...
function updateTitlebar() {
  try {
    ...
    const c = dark ? { color: '#050c11', symbolColor: '#d4d4d8' } : { color: '#e7edf4', symbolColor: '#0e2a47' };
    if (window.view && window.view.setTitlebar) window.view.setTitlebar(c);
  } catch {}
}
function searchUrl(q) {
  return (ENGINES[searchEngine] || ENGINES.onion).url + encodeURIComponent(q);
}
...
function createTab(url) {
  const tab = { id: ++seq, url: null, title: 'Ny flik', favicon: null, cleared: new Set(), bypassed: new Set(), canBack: false, canForward: false, verdict: null, warning: null, toastDismissed: new Set(), entering: true, overlay: null };
...
function saveOpenTabs() {
  try {
    const urls = tabs.filter((t) => !t._closing && t.url).map((t) => t.url);
...
function showActiveTab() {
  ...
  if (active && active.url) {
    ...
    window.view.hide();
    newtabpage.classList.remove('hidden'); greet();
  }
}
function switchTab(tab) {
  active = tab;
  $('search-engine').textContent = (ENGINES[searchEngine] || ENGINES.onion).label;
...
function renderTabs() {
  ...
  tabs.forEach((tab) => {
    ...
    el.className = 'tab' + (tab === active ? ' active' : '') + (tab.entering ? ' entering' : '');
...
function loadInTab(tab, url) {
  tab.url = url;
  tab.warning = null;
  if (tab === active) { newtabpage.classList.add('hidden'); hideInfobar(); addressInput.value = pretty(url); setShield(protectionOn ? 'checking' : 'off'); }
...
window.view.onTitle((id, title) => { const t = byId(id); if (t) { t.title = title; renderTabs(); if (t.url) pushHistory(t.url, title, t.favicon); } });
...
function renderSetHero() {
  ...
  if (!account) {
    ...
    nm.textContent = 'Forget'; em.textContent = 'Inte inloggad';
...
function renderEngines() {
  ...
  Object.keys(ENGINES).forEach((k) => {
    ...
    el.addEventListener('click', () => {
      ...
      if (active) $('search-engine').textContent = ENGINES[k].label;
      renderEngines();
    });
    list.appendChild(el);
  });
...
/* ── Tema (ljust/mörkt) — Forget är mörkt som standard ── */
let theme = 'dark';
// Engångs: tvinga mörkt vid första start av Forget (även på
// gamla profiler som råkat spara 'light'); användaren kan byta i Utseende sen.
try {
  if (!localStorage.getItem('forget-dark-init')) { localStorage.setItem('skoll-theme', 'dark'); localStorage.setItem('forget-dark-init', '1'); }
...
try { if (!localStorage.getItem('forget-bg-reset')) { localStorage.removeItem('skoll-bg'); localStorage.setItem('forget-bg-reset', '1'); } } catch {}
...
function matchEngine(v) {
  v = ('' + (v || '')).toLowerCase();
  if (/ahmia|onion|lök|lok/.test(v)) return 'onion';
...
$('settings-reset').addEventListener('click', () => {
  ...
  searchEngine = 'onion'; topSitesMode = 'favorites';
  applyStoredBg(); applyTopSitesMode(); renderEngines();
  if (active) $('search-engine').textContent = ENGINES.onion.label;
});
...
async function loadDailyImage() {
  // Forget: ingen extern dagsbild — RÖR INTE bakgrunden (den lila gradienten
...
// Forget: INGA färdiga genvägar — du lägger till dina egna.
...
// Engångs-nollställning för Forget: töm ev. gamla genvägar en gång.
try { if (!localStorage.getItem('forget-init-v1')) { localStorage.setItem('skoll-shortcuts', JSON.stringify(DEFAULT_SHORTCUTS)); localStorage.setItem('forget-init-v1', '1'); } } catch {}
...
function renderShortcuts() {
  ...
  (freq ? topSites() : getShortcuts()).forEach((sc, idx) => {
    ...
    // Forget: ingen favicon-hämtning från tredje part — den skulle avslöja för
    // ikon-servern vilka sajter du sparat. Bokstavsbrickan får räcka.
...
/* ── Uppdatering (går genom Tor, se updater.js) ─────────────────────────────
 * Tre lägen i samma remsa: ny version finns → hämtar (procent) → klar att
 * installera. Hämtningen startar aldrig av sig själv: filen är stor och går
 * genom Tor, så det ska vara ditt beslut. */
function updBar() {
  let bar = document.getElementById('update-banner');
  if (bar) return bar;
  bar = document.createElement('div'); bar.id = 'update-banner';
  bar.style.cssText = 'position:fixed;left:50%;transform:translateX(-50%);bottom:18px;z-index:300;display:flex;align-items:center;gap:12px;'
    + 'background:#08131a;color:var(--fgt-text,#dff5f1);border:1px solid rgba(126,240,220,.28);padding:11px 14px 11px 18px;border-radius:14px;'
    + 'box-shadow:0 14px 40px rgba(0,0,0,.55);font-size:13.5px;max-width:92vw;';
  document.body.appendChild(bar);
  return bar;
}
function mb(n) { return n ? Math.round(n / 1048576) + ' MB' : ''; }
window.view.onUpdateAvailable((d) => {
  const bar = updBar();
  bar.innerHTML = '<span>Ny version av Forget' + (d && d.version ? ' (' + d.version + ')' : '') + ' finns.</span>'
    + '<button id="upd-get" style="background:var(--fgt-accent,#7ef0dc);color:#04231f;border:0;border-radius:9px;padding:8px 14px;font-weight:700;font-size:13px;cursor:pointer;white-space:nowrap;">H\u00e4mta' + (d && d.size ? ' (' + mb(d.size) + ')' : '') + '</button>'
    + '<button id="upd-later" style="background:none;border:0;color:rgba(223,245,241,.65);font-size:13px;cursor:pointer;">' + escapeHtml(tt('Senare')) + '</button>';
  bar.querySelector('#upd-get').addEventListener('click', () => {
    bar.querySelector('#upd-get').textContent = 'H\u00e4mtar\u2026';
    try { window.view.downloadUpdate(); } catch {}
  });
  bar.querySelector('#upd-later').addEventListener('click', () => bar.remove());
});
window.view.onUpdateProgress((d) => {
  const bar = document.getElementById('update-banner'); if (!bar) return;
  bar.innerHTML = '<span>H\u00e4mtar Forget' + (d && d.version ? ' ' + d.version : '') + ' genom Tor\u2026 <b>' + ((d && d.pct) || 0) + ' %</b></span>'
    + '<span style="width:120px;height:6px;border-radius:99px;background:rgba(126,240,220,.15);overflow:hidden;display:inline-block">'
    + '<span style="display:block;height:100%;width:' + ((d && d.pct) || 0) + '%;background:var(--fgt-accent,#7ef0dc)"></span></span>';
});
window.view.onUpdateFailed(() => {
  const bar = document.getElementById('update-banner'); if (!bar) return;
  bar.innerHTML = '<span>Uppdateringen gick inte att h\u00e4mta. F\u00f6rs\u00f6k igen senare.</span>'
    + '<button id="upd-later" style="background:none;border:0;color:rgba(223,245,241,.65);font-size:13px;cursor:pointer;">St\u00e4ng</button>';
  bar.querySelector('#upd-later').addEventListener('click', () => bar.remove());
});
(function () {
  const b = document.getElementById('upd-check');
  if (b) b.addEventListener('click', async () => {
    b.textContent = 'Kollar\u2026'; b.disabled = true;
    try { await window.view.checkUpdate(); } catch {}
    setTimeout(() => { b.textContent = 'S\u00f6k efter uppdatering'; b.disabled = false; }, 1200);
  });
})();
...
function showUpdateBanner(version) {
  ...
  bar.innerHTML = '<span>Forget' + (version ? ' ' + version : '') + ' ' + escapeHtml(tt('är hämtad och klar.')) + '</span>'
    + '<button id="upd-now" style="background:var(--fgt-accent,#7ef0dc);color:#04231f;border:0;border-radius:9px;padding:8px 14px;font-weight:700;font-size:13px;cursor:pointer;white-space:nowrap;">' + escapeHtml(tt('Starta om & uppdatera')) + '</button>'
    + '<button id="upd-later" style="background:none;border:0;color:rgba(223,245,241,.65);font-size:13px;cursor:pointer;">Senare</button>';
...
(function initTor() {
  ...
  function render(s) {
    ...
    if (s.phase === 'ready') {
      ...
      if (errEl) errEl.textContent = s.error || 'Okänt fel — starta om appen.';
      ...
      // Forget bygger upp Tor från noll varje start (inget sparas mellan
      // sessionerna) → säg det, annars ser en långsam uppkoppling ut som ett fel.
      if (sub && p >= 25) sub.textContent = 'Bygger nya kretsar från noll — ingenting sparas mellan sessionerna.';
    }
  }
